...
THE_HOST = "https://tanda.telegate.net.au"
...
var currAccount = getRest(THE_HOST + "/rest/domain/self/account/self/list/list_identity").user;
  ...
  if (!document.hasFocus()) {
    if ("Notification" in window) {
      if (Notification.permission !== 'denied') {
        var chatnotify = new Notification(user.name, { icon: THE_HOST + "rest/domain/self/account/" + user.account + "/image/picture/150x150?" + new Date().getTime(), body: msg });
...
function setViewSettings() {
  ...
  var trans = getRest(THE_HOST + "/rest/system/translation/" + session.lang + "/usr_core_settings.htm,usr_index.js,-");
...
function saveView() {
  ...
  putRest(THE_HOST + "/rest/domain/self/account/self/user_settings", saveData);
}

function cancelView() {
  var data = getRest(THE_HOST + "/rest/domain/self/account/self/user_settings");
...
function getUserElement(user) {
  ...
  if(refreshImages)
    img.src = THE_HOST + "rest/domain/self/account/" + user.account + "/image/picture/150x150?" + new Date().getTime();
  else
    img.src = THE_HOST + "rest/domain/self/account/" + user.account + "/image/picture/150x150";
  td2.appendChild(img);
  tr2.appendChild(td2);
